...
const Header = () => {
    ...
    const userButtons = () => {
        if (!localStorage.getItem("token")) {
            ...
            return <Link to='/dashboard' style={{textDecoration:'none', color:'white'}}><h5 style={{ color: "white" }}>{auth.user?.fullName}</h5></Link>;
        }
    };
    ...
    return (
        <nav className="sticky-top navbar navbar-expand-lg navbar-dark bg-dark">
            ...
            >
                ...
                <div className="form-inline my-2 my-lg-0">{userButtons()}</div>
            </div>
        </nav>
    );
};
