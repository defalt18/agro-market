...
exports.signIn = (req, res) => {
	User.findOne({ contactNumber: req.body.contactNumber }).exec((err, user) => {
		if (err) {
...
exports.updateUser = (req, res) => {
	User.findById(req.params.id).exec((err, _user) => {
		...
		if (req.body.fullName != null) _user.fullName = req.body.fullName

		if (req.body.email != null) _user.email = req.body.email

		if (req.body.contactNumber != null)
			_user.contactNumber = req.body.contactNumber

		if (req.body.address != null) _user.address = req.body.address

		if (req.body.city != null) _user.city = req.body.city

		if (req.body.state != null) _user.state = req.body.state

		if (req.body.pincode != null) _user.pincode = req.body.pincode
...
exports.reviewUpdate = (req, res) => {
	...
	User.findById(req.params.id).exec((err, _user) => {
		...
		if (req.body.addReview) {
			...
			totRating =
				(totRating + req.body.addReview.rating) / (_user.reviews.length + 1)
		...
		if (req.body.delReview) {
			let totRating = _user.ratings * _user.reviews.length
			totRating =
				(totRating - req.body.delReview.rating) / (_user.reviews.length - 1)
